...
# ==========================================
# RUTEO DE PÁGINAS (DASHBOARD REDISEÑADO - VERSIÓN SEGURA)
# ==========================================
if st.session_state.current_page == "home":
    
    # 1. Textos descriptivos dinámicos según el idioma
    ...
    # 2. CSS PARA LAS TARJETAS VISUALES (100% Seguro, no interfiere con Streamlit)
    st.markdown("""
    <style>
    .dash-card {
        background: #FFFFFF;
        ...
        padding: 25px 20px;
        text-align: center;
        box-shadow: 8px 8px 16px rgba(0,0,0,0.04), -8px -8px 16px rgba(255,255,255,0.5);
    ...
    .dash-card:hover {
        ...
        box-shadow: 12px 12px 24px rgba(16,185,129,0.15);
        border-color: #10B981;
    }
    .dash-icon { width: 80px; height: 80px; margin-bottom: 15px; }
    .dash-title { font-size: 22px; font-weight: 800; color: #1E293B; margin: 0 0 8px 0; }
    .dash-desc { font-size: 14px; color: #64748B; margin: 0; line-height: 1.4; }
    </style>
    """, unsafe_allow_html=True)

    # 3. Renderizado de Título
    ...
    # 4. Diseño del Menú 2x2 (Tarjetas Visuales + Botones Nativos)
    c1, c2 = st.columns(2)
    
    with c1:
        # TARJETA 1: COCINA MÁGICA
        st.markdown(f"""
        <div class="dash-card">
            <svg class="dash-icon" viewBox='0 0 24 24' fill='none' stroke='#10b981' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'>
                <path d='M6 13.87A4 4 0 0 1 7.41 6a5.11 5.11 0 0 1 1.05-1.54 5 5 0 0 1 7.08 0A5.11 5.11 0 0 1 16.59 6 4 4 0 0 1 18 13.87V21H6Z'/>
                <line x1='6' y1='17' x2='18' y2='17'/>
                <g><animateTransform attributeName='transform' type='rotate' values='-10 12 12; 10 12 12; -10 12 12' dur='2s' repeatCount='indefinite'/><circle cx='12' cy='3' r='1' fill='#10b981'><animate attributeName='opacity' values='0;1;0' dur='1.5s' repeatCount='indefinite'/></circle></g>
            </svg>
            <h3 class="dash-title">{t["dash_mod1"]}</h3>
            <p class="dash-desc">{d_m1}</p>
        </div>
        """, unsafe_allow_html=True)
        if st.button("🪄 Entrar a Cocina Mágica", use_container_width=True, key="btn_m1"):
            st.session_state.current_page = "mod1"
            st.session_state.step = "input"
            st.rerun()
        ...
        st.markdown(f"""
        <div class="dash-card">
            <svg class="dash-icon" viewBox='0 0 24 24' fill='none' stroke='#f59e0b' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'>
                <rect x='3' y='4' width='18' height='18' rx='2' ry='2'/>
                <line x1='16' y1='2' x2='16' y2='6'/><line x1='8' y1='2' x2='8' y2='6'/><line x1='3' y1='10' x2='21' y2='10'/>
                <path d='M8 15 l2 2 l4 -4'><animate attributeName='stroke-dasharray' values='0,20; 20,0; 20,0' dur='2s' repeatCount='indefinite'/></path>
            </svg>
            <h3 class="dash-title">{t["dash_mod3"]}</h3>
            <p class="dash-desc">{d_m3}</p>
        </div>
        """, unsafe_allow_html=True)
        if st.button("📅 Abrir Mis Menús", use_container_width=True, key="btn_m3"):
            st.session_state.current_page = "mod3"
            st.rerun()

    with c2:
        # TARJETA 2: MI COMPRA (STICKMAN ANIMADO)
        st.markdown(f"""
        <div class="dash-card">
            <svg class="dash-icon" viewBox='0 0 60 40' stroke='#3b82f6' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' fill='none'>
                <g>
                    <animateTransform attributeName='transform' type='translate' values='-3,0; 3,0; -3,0' dur='1.2s' repeatCount='indefinite'/>
                    <path d='M28 15 h6 l4 10 h12'/>
                    <line x1='32' y1='25' x2='48' y2='25'/>
                    <circle cx='36' cy='30' r='2' fill='#3b82f6'/><circle cx='46' cy='30' r='2' fill='#3b82f6'/>
                    <circle cx='18' cy='8' r='3'/><path d='M18 11 v10'/>
                    <path d='M18 21 l-4 8'><animate attributeName='d' values='M18 21 l-4 8; M18 21 l4 8; M18 21 l-4 8' dur='0.4s' repeatCount='indefinite'/></path>
                    <path d='M18 21 l6 7'><animate attributeName='d' values='M18 21 l6 7; M18 21 l-2 8; M18 21 l6 7' dur='0.4s' repeatCount='indefinite'/></path>
                    <path d='M18 14 l10 3'/>
                </g>
            </svg>
            <h3 class="dash-title">{t["dash_mod2"]}</h3>
            <p class="dash-desc">{d_m2}</p>
        </div>
        """, unsafe_allow_html=True)
        if st.button("🛒 Ver Mi Compra", use_container_width=True, key="btn_m2"):
        ...
        st.markdown(f"""
        <div class="dash-card">
            <svg class="dash-icon" viewBox='0 0 24 24' fill='#ef4444' stroke='#ef4444' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'>
                <path d='M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z'>
                    <animate attributeName='transform' type='scale' values='1;1.15;1;1;1' dur='1.2s' repeatCount='indefinite' transform-origin='12 12'/>
                </path>
                <polyline points='9 12 11 12 12 9 13 15 14 12 16 12' stroke='white' stroke-width='1' fill='none'>
                    <animate attributeName='stroke-dasharray' values='0,20; 20,0; 20,0' dur='2s' repeatCount='indefinite'/>
                </polyline>
            </svg>
            <h3 class="dash-title">{t["dash_mod4"]}</h3>
            <p class="dash-desc">{d_m4}</p>
        </div>
        """, unsafe_allow_html=True)
        if st.button("🩺 Revisar Mi Salud", use_container_width=True, key="btn_m4"):
