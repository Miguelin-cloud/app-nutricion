...
if st.session_state.current_page == "home":
    # Textos descriptivos dinámicos para el efecto Hover según el idioma actual
    if lang_code == "Spanish":
        d_m1 = "Mágicamente descubre recetas saludables usando los ingredientes de tu despensa."
        d_m2 = "Gestiona tu inventario y extrae ingredientes web con Inteligencia Artificial."
        d_m3 = "Planifica tu semana, ajusta macros y lleva el control preciso de cada comida."
        d_m4 = "Evalúa tu salud con reportes clínicos detallados generados por IA."
    elif lang_code == "French":
        d_m1 = "Découvrez des recettes magiques avec les ingrédients de votre cuisine."
        d_m2 = "Gérez votre inventaire et extrayez des ingrédients web avec l'IA."
        d_m3 = "Planifiez votre semaine et surveillez vos macros pour chaque repas."
        d_m4 = "Évaluez votre santé avec des rapports cliniques générés par l'IA."
    elif lang_code == "Italian":
        d_m1 = "Scopri ricette magiche con gli ingredienti della tua dispensa."
        d_m2 = "Gestisci il tuo inventario ed estrai ingredienti dal web con l'IA."
        d_m3 = "Pianifica la tua settimana e controlla i macro di ogni pasto."
        d_m4 = "Valuta la tua salute con report clinici generati dall'IA."
    else: # English
        d_m1 = "Magically discover healthy recipes using your pantry ingredients."
        d_m2 = "Manage your grocery inventory and extract ingredients using AI."
        d_m3 = "Plan your week, adjust macros, and track every single meal."
        d_m4 = "Evaluate your health with detailed clinical reports generated by AI."

    # CSS con EFECTO NEUMORFISMO, HOVER MÁGICO Y SVGs ANIMADOS INYECTADOS
    st.markdown(f"""
    <style>
    /* Diseño Base de los Botones (Neumorfismo Suave) */
    [data-testid="column"] div.stButton > button {{
        ...
        /* Sombra Neumórfica 3D (luz arriba-izquierda, sombra abajo-derecha) */
        box-shadow: 10px 10px 20px #e2e3e4, -10px -10px 20px #ffffff !important;
    ...
    /* EFECTO HOVER: Flota más alto, el ícono sube, el título sube y aparece la descripción */[data-testid="column"] div.stButton > button:hover {{
        transform: translateY(-8px) scale(1.02) !important;
        box-shadow: 18px 18px 36px #d5d6d8, -18px -18px 36px #ffffff !important;
    ...
    /* Pseudo-elemento para las Descripciones (Aparecen por arte de magia) */[data-testid="column"] div.stButton > button::after {{
        position: absolute;
        bottom: 20px;
        left: 15px;
        right: 15px;
        ...
        line-height: 1.4;
        z-index: 1;
    }}[data-testid="column"] div.stButton > button:hover::after {{
        opacity: 1;
        transform: translateY(0);
    }}
    
    /* 1: Cocina Mágica (Gorro Chef & Brillos Girando) */
    ...
    [data-testid="column"]:nth-of-type(1) div.stButton:nth-of-type(1) button::after {{ content: "{d_m1}"; }}
    
    /* 3: Mis Menús (Calendario que se actualiza solo) -> Botón 2 Columna 1 */
    ...
    [data-testid="column"]:nth-of-type(1) div.stButton:nth-of-type(2) button::after {{ content: "{d_m3}"; }}

    /* 2: Mi Compra (El Stickman Corriendo con el carrito) -> Botón 1 Columna 2 */
    ...
    [data-testid="column"]:nth-of-type(2) div.stButton:nth-of-type(1) button::after {{ content: "{d_m2}"; }}

    /* 4: Mi Salud (Corazón Latiendo tipo Médico) -> Botón 2 Columna 2 */
    [data-testid="column"]:nth-of-type(2) div.stButton:nth-of-type(2) button {{
    ...
    [data-testid="column"]:nth-of-type(2) div.stButton:nth-of-type(2) button::after {{ content: "{d_m4}"; }}
    
    /* Ajuste en modo móvil */
    @media (max-width: 768px) {{
        [data-testid="column"] div.stButton > button {{ min-height: 200px !important; font-size: 20px !important; margin-bottom: 15px; }}
        [data-testid="column"] div.stButton > button::after {{ font-size: 12px !important; bottom: 10px; }}[data-testid="column"] div.stButton > button:hover p {{ transform: translateY(-15px) !important; }}
    }}
    </style>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.cal_view == "weekly":
        ...
        for i, d in enumerate(days):
            with cols[i]:
                ...
                st.markdown(f"<div style='text-align:center; font-weight:bold; padding:8px; background:{bg_title}; color:{txt_color}; border-radius:8px; margin-bottom:10px;'>{d.strftime('%a %d')}</div>", unsafe_allow_html=True)
